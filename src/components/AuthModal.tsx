...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3.5 sm:p-4 bg-black/65 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-2xl sm:rounded-3xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden flex flex-col transition-all">
        {/* Header */}
        <div className="relative bg-gradient-to-tr from-pink-600 via-rose-600 to-indigo-600 p-5 sm:p-6 text-white text-center">
          <button
          ...
          <p className="text-xs sm:text-sm text-pink-100 mt-1 max-w-xs mx-auto">
            {initialReason === 'limit_reached'
              ? `You've completed your 3 free guest downloads. Enter your email to unlock unlimited HD downloads!`
              : 'Enter your email to unlock unlimited downloads and access your download history.'}
          </p>
        </div>
        ...
        <div className="p-4 sm:p-6 space-y-4">
          {initialReason === 'limit_reached' && (
            <div className="bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 rounded-xl p-3 flex items-center justify-between text-xs text-rose-700 dark:text-rose-300">
              <span className="font-semibold">Visitor Downloads Used:</span>
              <span className="font-bold bg-rose-200 dark:bg-rose-900 px-2 py-0.5 rounded-full text-rose-900 dark:text-rose-100">
                {downloadCount} / 3
              </span>
            </div>
          )}
          ...
          {error && (
            <div className="p-2.5 sm:p-3 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 rounded-xl text-rose-600 dark:text-rose-300 text-xs font-medium">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              ...
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 sm:py-2.5 bg-zinc-50 dark:bg-zinc-800/80 border border-zinc-200 dark:border-zinc-700 rounded-xl text-xs sm:text-sm text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 outline-none transition-all"
                  autoFocus
                />
              </div>
            </div>
            ...
            <div className="pt-2 space-y-1.5 text-zinc-600 dark:text-zinc-400 text-xs">
              <div className="flex items-center gap-2">
                ...
                <span>Unlimited Full HD downloads without watermarks</span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
              ...
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                <span>Zero fees and saved download history</span>
              </div>
            </div>
            ...
            <button
              ...
              type="submit"
              disabled={isLoading}
              className="w-full mt-2 inline-flex items-center justify-center gap-2 bg-gradient-to-r from-pink-600 via-rose-600 to-indigo-600 hover:from-pink-700 hover:via-rose-700 hover:to-indigo-700 text-white font-bold text-xs sm:text-sm py-2.5 sm:py-3 rounded-xl shadow-lg shadow-rose-600/20 active:scale-[0.99] transition-all cursor-pointer disabled:opacity-60"
            >
              {isLoading ? (
                <>
