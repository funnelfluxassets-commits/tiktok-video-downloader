...
import { X, Trash2, ArrowUpRight, Clock, Film, Image as ImageIcon } from 'lucide-react';
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 divide-y divide-zinc-100 dark:divide-zinc-800">
          {history.length === 0 ? (
            <div className="text-center py-10 sm:py-12">
              <Film className="w-10 h-10 sm:w-12 sm:h-12 text-zinc-300 dark:text-zinc-700 mx-auto mb-3" />
              <p className="font-semibold text-zinc-800 dark:text-zinc-200 text-sm">No recent downloads yet</p>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
                Paste any TikTok URL to download and your history will be saved here.
              </p>
            </div>
          ) : (
            history.map((item) => (
              ...
              >
                <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                  <div className="relative w-10 h-14 sm:w-12 sm:h-16 rounded-lg overflow-hidden bg-zinc-900 shrink-0 border border-zinc-200 dark:border-zinc-700">
                    <img
                      src={item.cover}
                      alt={item.title}
                    ...
                    />
                    {item.isPhotoSlide && (
                      <span className="absolute top-1 left-1 bg-black/60 p-0.5 rounded text-white">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] sm:text-[11px] text-zinc-500 dark:text-zinc-400 truncate">@{item.authorUsername}</p>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 text-[11px] sm:text-xs font-semibold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/70 hover:bg-rose-100 dark:hover:bg-rose-900/60 border border-rose-200 dark:border-rose-800/50 px-2.5 sm:px-3 py-1.5 rounded-lg transition-colors shrink-0 cursor-pointer"
                >
                  <span>Re-open</span>
        ...
        {history.length > 0 && (
          <div className="px-4 sm:px-6 py-3 bg-zinc-50 dark:bg-zinc-800/80 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
            <button
              ...
              onClick={onClear}
              className="inline-flex items-center gap-1.5 text-xs font-medium text-rose-600 dark:text-rose-400 hover:text-rose-700 hover:bg-rose-50 dark:hover:bg-rose-950/50 px-2.5 sm:px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
            >
